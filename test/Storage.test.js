...
contract("Donation", (accounts) => {
  ...
  describe("Posts", () => {
    it("allows the creator to add a post to the request", async () => {
      await donation.addPost("medical", 0, "Update", "desc", '19 Mar 2022 00:00:00 UTC');
      assert.equal((await donation.Posts("medical", 0, 0)).title, "Update");
    });
    it("should only let the creator add a post to the request", async () => {
      await donation.addPost("medical", 0, "Update", "desc", '19 Mar 2022 00:00:00 UTC',
      { from: accounts[1] }).should.be.rejectedWith(
        "VM Exception while processing transaction: revert"
      );
